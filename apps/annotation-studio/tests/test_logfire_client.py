...
from annotation_studio.logfire_client import parse_interaction, validate_trace_and_span
...
def test_build_trace_link_lookback_window_matches_lookback_days() -> None:
    # The link's `last=` window must cover everything a queue refresh can ever return (its
    # own min_timestamp bound, in routes.py's _run_refresh) — otherwise "Open trace in
    # Logfire" could open to an empty page for a trace this app itself is showing.
    url = logfire_client.build_trace_link("https://logfire-us.pydantic.dev", "duncan", "rx-assistant-demo", "abc123")
